...
class NoAnimation:
    ...
    def record_trag(self):
        self.trag.append(self.current)
    ...
    def run_game(self, path=None):
        ...
        if self.current == self.goal:  # if user clicked close
            print("agent reached goal, quitting")
        ...
        if self.done:# or (self.current_step % 1 == 0 and self.current_step != 0):
            grid = np.copy(self.world.occupancy_grid_map)

            for i in range(grid.shape[0]):
                for ii in range(grid.shape[1]):
                    if grid[i][ii] == OBSTACLE_GRID:
                        grid[i][ii] = 1
            for i in self.trag:
                grid[i] = 2
...
class Animation:
    ...
    def run_game(self, path=None):
        ...
        for event in pygame.event.get():
            ...
            elif True or (event.type == pygame.KEYDOWN and event.key == pygame.K_SPACE) or self.cont :
                # space bar pressed. call next action
                if path:
                    (x, y) = path[1]
                    self.set_position((x, y))
        ...
        for row in range(self.x_dim):
            for column in range(self.y_dim):
                # color the cells
                pygame.draw.rect(self.screen, colorsx[self.world.occupancy_grid_map[row][column]],
